...
% % Float behavior tuning: keep floats with text; avoid float-only pages
% \makeatletter
% \setlength{\textfloatsep}{8pt plus 2pt minus 2pt}  % space above/below floats
% \setlength{\floatsep}{8pt plus 2pt minus 2pt}
% \setlength{\intextsep}{8pt plus 2pt minus 2pt}
% \renewcommand{\topfraction}{0.85}
% \renewcommand{\bottomfraction}{0.70}
% \renewcommand{\textfraction}{0.10}
% \renewcommand{\floatpagefraction}{0.80}  % don't make a float-only page unless >=80% full
...
\begin{tikzpicture}[
  ...
  \node[ann] at (-0.9,-2.8) {In-enclave DKG and RA$^{(\mathrm{DKG})}$};
  ...
  \draw[msg] ($(C)+(0,-5.1)$) -- node[above,sloped]{broadcast $(m^{\star},\mathsf{sid})$} ($(En)+(0,-5.1)$);
...
% \noindent
% $\textsf{RA.GenQuote}(\mathsf{aux}) \to \mathsf{RA}$\\
% $\textsf{RA.Verify}(\mathsf{RA};\ \textsf{allow\_meas},\ \textsf{roots}) \in \{0,1\}$\\
% $\textsf{Fresh}(\mathsf{RA},\sid) \in \{0,1\}$\\
% $\textsf{Latch}(\sid,m^{\star})$\\
% $\textsf{Del}()$
...
% This yields weaker, but composable, guarantees and can be \emph{combined} with RA in hybrid QuanTEEum instantiations for stronger additivity.
